...
export default class ClientViewModel {
  static async getStatsForClient(client: Client): Promise<ClientStats> {
    const { id } = client;
    const clientId = id?.toString() as string;
    ...
    const activeLoanDebt = activeLoanList.reduce((total, loan) => loan.currentBalance + total, 0);
